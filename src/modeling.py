...
class ImageClassifier(torch.nn.Module):
    ...
    def regularization(self, pretrain):
        curr = torch.cat([torch.flatten(params) for params in self.parameters()])
        pret = torch.cat([torch.flatten(params)] for params in pretrain.parameters())
        return torch.norm(curr-pret)
